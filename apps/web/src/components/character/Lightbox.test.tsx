...
function renderLightbox(index = 0) {
  return render(
    <Lightbox artworks={artworks} index={index} onClose={onClose} onNavigate={onNavigate} />
  );
}
...
describe("Lightbox", () => {
  it("renders the image and metadata for the current artwork", () => {
    ...
    expect(
      screen.getByText("Portrait commission for The Shattered Crown campaign.")
    ).toBeInTheDocument();
  ...
  it("omits metadata block when all fields are null", () => {
    ...
    render(<Lightbox artworks={noMeta} index={0} onClose={onClose} onNavigate={onNavigate} />);
  ...
  it("Prev and Next buttons are enabled in the middle", () => {
    ...
    render(<Lightbox artworks={three} index={1} onClose={onClose} onNavigate={onNavigate} />);
